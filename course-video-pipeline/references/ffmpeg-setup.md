...
-->
# Terminal Merge — FFmpeg Setup and Commands
...
ffmpeg -version
```
If you see version info, you're ready. If not, install below.
...
```
Then restart your terminal. If winget isn't available, use the manual method below.

### Windows 10 (manual)
1. Download a pre-built binary from https://www.gyan.dev/ffmpeg/builds/
...
3. Add `C:\ffmpeg\bin` to your PATH:
   ...
   - Add `C:\ffmpeg\bin`
4. Restart terminal
5. Run `ffmpeg -version` to confirm
...
## merge.py — cross-platform merge script

Save this script as `merge.py` in the same folder as your video and audio files.
Run it with: `python merge.py [slug]`
...
# Edit END_CARD_LINES in merge.py first, then run:
python merge.py ugst1001-p2s1-analogy-forgetting-curve
```

Output: `[slug].mp4` in the same folder.
